...
slate.Variants = (function() {
  ...
  Variants.prototype = _.assignIn({}, Variants.prototype, {
    ...
    /**
     * 
     * Determine whether options in variant 2 are in stock based on the currently selected variant 1 (for adding Strikethrough on unavailable size variants)
     * 
     */
    _isSizeAvailable: function() {
      if (!document.querySelector('.size-variant-function-run')) {
        var s = document.querySelectorAll('.real-size-selector__item')
        for (var j = 0; j < s.length;j++) {
          s[j].classList.add('size-variant-function-run');
        }
      }
      // Remove existing classes variants  -- i.e start fresh
      var av = document.querySelectorAll('size-variant-available')
      for (var i = 0; i < av; i++) {
        av.classList.remove('size-variant-available')
      }
      ...
      for (var j = 0; j < currentOptions.length;j++) {
        if (currentOptions[j].index === 'option1') { // Where option 1 === color
          color = currentOptions[j].value;
          break;
        }
      }
      if (color) {
        for (var i = 0; i < variants.length; i++) {
          if (variants[i].option1 !== color) continue;
          
          // where size-variant-id-{{option}} naming is specified in product-template.liquid
          if (variants[i].available)
            document.getElementById(`size-variant-id-${variants[i].option2}`).classList.add('size-variant-available');
        }
      }
...
theme.Video = (function() {
  ...
  function init($video) {
    ...
    videos[$video.attr('id')] = {
      ...
      alwaysAutoplay:$video.data('always-autoplay'),
      status:
...
$(document).ready(function() {
  ...
  sections.register('custom-gallery', theme.VideoSection);
  sections.register('hero-video', theme.VideoSection);
...
if (document.getElementById('color-variant-value')) {
  document.getElementById('color-variant-value').innerHTML = val;
}
$('input[type=radio][name="color"]').change(function() {
  if (document.getElementById('color-variant-value')) {
    document.getElementById('color-variant-value').innerHTML = this.value;
  }
});

document.getElementById('nav-icon3').addEventListener('click', function() {
  this.classList.toggle('open');
  document.getElementById('AccessibleNav').classList.toggle('open')
